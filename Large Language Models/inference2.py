...
def caption_video_segment(video_path: str, min_len: int = 10, max_len: int = 20) -> str:
    ...
    try:
        ...
        seg_len = container.streams.video[0].frames or int(
            (float(container.streams.video[0].duration) / container.streams.video[0].time_base) if
            container.streams.video[0].duration else 0
        )
    except Exception:
        seg_len = 0
...
# ✅ Modifications intégrées pour forcer la génération de caption sur toute la vidéo (si aucune scène > `min_scene_len`) ET inclure le caption global au début de la vidéo exportée.

# =====================
# Ajouts dans process_video_with_scenes()
# =====================
...
def process_video_with_scenes(
    ...
    crf: int = 18,
    preset: str = "medium",
    force_global_caption: bool = True,  # <--- nouveau param
) -> dict:
    ...
    with VideoFileClip(video_path) as video:
        if not spans or force_global_caption:
            # Forcer un seul bloc si aucune scène ou si demandé
            spans = [(0.0, float(video.duration))]
    ...
    if burn_subtitles:
        clips = []
        with VideoFileClip(video_path) as video:
            # Caption global au début (si utile)
            if force_global_caption:
                intro_clip = video.subclip(0, min(5.0, video.duration))
                intro = overlay_caption_on_clip(intro_clip, global_caption)
                clips.append(intro)
...
if __name__ == "__main__":
    path = "video"
    input_videos = [
        os.path.join(path, "test1.mp4"),
        # os.path.join(path, "test2.mp4"),
        # os.path.join(path, "video31.mp4"),
    ...
    for vp in input_videos:
        ...
        print(f"⏱ Temps de traitement pour {base}: {elapsed:.2f} secondes")
        print("Captions exportés:", outputs["json"])
        print("SRT exporté:", outputs["srt"])
        if outputs["final_video"]:
            print("Vidéo légendée:", outputs["final_video"])
        print("Caption global:", outputs["global_caption"])

        # --- Sauvegarde d'un résumé texte du run ---
        out_dir = "result"  # pas de virgule !
        log_path = os.path.join(out_dir, f"{base}_summary.txt")
        with open(log_path, "w", encoding="utf-8") as f:
            ...
            # pas de global_caption_brief dans cette version
            f.write(f"Caption global: {outputs['global_caption']}\n")

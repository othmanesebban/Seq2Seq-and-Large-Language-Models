...
def process_video_with_scenes(
    ...
    json_path = os.path.join(out_dir, f"{base}_captions_with_filtrage.json")
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(meta, f, ensure_ascii=False, indent=2)

    srt_path = os.path.join(out_dir, f"{base}_with_filtrage.srt")
    write_srt(spans, per_scene_caps, srt_path)

    # 5) Vidéo finale avec sous-titres par scène
    final_video_path = os.path.join(out_dir, f"{base}_with_scene_captions__with_filtrage.mp4")
    if burn_subtitles:
        clips = []
        with VideoFileClip(video_path) as video:
...
if __name__ == "__main__":
    ...
    for vp in input_videos:
        ...
        log_path = os.path.join(out_dir, f"{base}_summary_with_filtrage.txt")

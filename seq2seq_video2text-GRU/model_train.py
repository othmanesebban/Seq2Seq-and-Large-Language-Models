...
test_dir = 'MLDS_hw2_1_data/testing_data'
...
class vedio2text:
    ...
    def build_model(self, feat, captions=None, cap_len=None, sampling=None, phase=0):
        ...
        with tf.variable_scope('GRU1'):
            gru_red = tf.nn.rnn_cell.GRUCell(n_hidden)
            if phase == phases['train']:
                gru_red = tf.contrib.rnn.DropoutWrapper(gru_red, output_keep_prob=dropout_prob)
        with tf.variable_scope('GRU2'):
            gru_gre = tf.nn.rnn_cell.GRUCell(n_hidden)
            if phase == phases['train']:
                gru_gre = tf.contrib.rnn.DropoutWrapper(gru_gre, output_keep_prob=dropout_prob)

        state_red = gru_red.zero_state(batch_size, dtype=tf.float32)
        state_gre = gru_gre.zero_state(batch_size, dtype=tf.float32)
        ...
        for i in range(0, n_frames):
            with tf.variable_scope("GRU1"):
                output_red, state_red = gru_red(image_emb[i, :, :], state_red)

            with tf.variable_scope("GRU2"):
                output_gre, state_gre = gru_gre(tf.concat([padding, output_red], axis=1), state_gre)
        ...
        for i in range(0, max_caption_len):
            with tf.variable_scope("GRU1"):
                output_red, state_red = gru_red(padding_in, state_red)

            if i == 0:
                with tf.variable_scope("GRU2"):
                    con = tf.concat([bos, output_red], axis=1)
                    output_gre, state_gre = gru_gre(con, state_gre)
            else:
                if phase == phases['train']:
                    if sampling[i] == True:
